      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        >
          ...
          <p className="text-lg md:text-xl text-white/70 max-w-3xl mx-auto">
            Explore our archive of successful events and workshops
          </p>
        </motion.div>
        ...
        {loading ? (
          <div className="text-center py-20">
            ...
            <p className="mt-4 text-white/70">Loading past events...</p>
          </div>
        ) : filteredEvents.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-xl text-white/70">No past events found</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {filteredEvents.map((event, index) => (
              <motion.div
                ...
                className="group relative bg-gradient-to-br from-white/5 to-white/[0.02] border border-white/10 rounded-2xl overflow-hidden hover:border-white/20 transition-all duration-300 flex flex-col h-full grayscale hover:grayscale-0 opacity-70 hover:opacity-100"
                style={{ transition: 'filter 0.5s ease, opacity 0.5s ease' }}
              >
                {/* Event Image */}
                {event.banner_image && (
                  <div className="relative h-48 sm:h-56 md:h-64 overflow-hidden bg-black/20 flex-shrink-0">
                    ...
                    <div className="absolute top-2 sm:top-3 right-2 sm:right-3">
                      <span className="px-2 sm:px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-xs font-medium text-white border border-white/20">
                        {event.category}
                      </span>
                    </div>
                  </div>
                ...
                <div className="p-4 sm:p-5 md:p-6 flex flex-col flex-grow">
                  <h3 className="text-base sm:text-lg md:text-xl font-bold mb-2 text-white group-hover:text-rose-300 transition-colors line-clamp-2">
                    {event.title}
                  </h3>
                  ...
                  <button
                    ...
                    className="w-full group/view relative px-3 sm:px-4 py-2 sm:py-2.5 bg-gradient-to-r from-indigo-500/20 to-rose-500/20 border border-indigo-300/30 hover:border-rose-300/50 rounded-xl transition-all duration-300 flex items-center justify-center gap-2 overflow-hidden mt-auto"
                  >
