...
export function UpcomingEventsPage(props: UpcomingEventsPageProps) {
  const [search, setSearch] = useState("");
  const [selectedEvent, setSelectedEvent] = useState<any>(null);
  const events = useQuery(api.events.getByStatus, { status: "upcoming" });
          ...
          {events === undefined ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-white/40 text-xl">
                {search ? "No events match your search" : "No upcoming events at the moment"}
              </p>
            </div>
          ) : (
                ...
                >
                  ...
                  {event.banner_image && (
                    <div className="relative h-48 overflow-hidden flex-shrink-0">
                      ...
                      <div className="absolute bottom-3 left-3">
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium backdrop-blur-sm bg-green-500/20 text-green-200 border border-green-500/30">
                          upcoming
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    ...
                    {event.application_deadline && (
                      <div className="mb-4 text-xs text-amber-400/70">
                        Deadline: {new Date(event.application_deadline).toLocaleDateString()}
                      </div>
                    )}
                    <div className="flex gap-2">
                      ...
                      <div onClick={(e) => e.stopPropagation()}>
                        <EventRegistrationButton eventId={event._id} onLoginRequired={props.onLoginClick} onProfileRequired={props.onProfileClick} />
                      </div>
                    </div>
                  </div>
                  </div>
